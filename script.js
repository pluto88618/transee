...
const DEEPSEEK_CONFIG = {
    ...
    model: 'deepseek-chat', // 主要翻译模型
    languageDetectionModel: 'deepseek-reasoner', // 语言检测专用模型
    maxRetries: 1, // 减少重试次数，提高速度
    retryDelay: 200, // 减少重试延迟
    timeout: 15000, // 减少超时时间到15秒
    batchSize: 500, // 减少批次大小，提高响应速度
    enableCache: true
};
...
function handleTranslateButtonClick() {
    ...
    if (AppState.uploadedFiles.length > 0 && !AppState.isProcessing) {
        ...
        showNotification('请先选择文件或等待当前处理完成', 'warning');
    }
}
...
function updateTranslationButtonState() {
    if (Elements.translateBtn && Elements.selectedFileCount && Elements.totalFileSize) {
        ...
        if (fileCount > 0 && !AppState.isProcessing) {
            ...
            Elements.translateBtn.textContent = '翻译处理中...';
        } else {
            Elements.translateBtn.disabled = true;
            Elements.translateBtn.textContent = '开始翻译';
        }
...
function processSelectedFiles(files) {
    ...
    if (AppState.uploadedFiles.length > 0) {
        ...
        showNotification('文件上传完成，请点击"开始翻译"按钮开始处理', 'success');
    }
}
...
async function startTranslationProcess() {
    ...
    try {
        ...
        const extractedText = await extractTextFromFiles(); // 移除模拟延迟，直接处理
        
        // 步骤2: 语言检测
        updateProcessingStatus('正在检测文档语言...', 40);
        if (sourceLang === 'auto' && Elements.useAITranslation.checked) {
            try {
                // 使用DeepSeek进行智能语言检测，只发送前1200字符减少API请求体积
                ...
                console.log(`✅ DeepSeek语言检测结果: ${AppState.languages[detectedLang] || detectedLang}`);
            } catch (langDetectError) {
                console.warn('⚠️ DeepSeek语言检测失败，回退到模拟检测:', langDetectError);
                AppState.detectedSourceLang = simulateLanguageDetection();
            }
        } else {
            AppState.detectedSourceLang = simulateLanguageDetection(); // 回退或手动选择
        }
...
/**
 * 使用DeepSeek API进行智能语言检测 - 超快速版本
 */
async function detectLanguageWithDeepSeek(text) {
    console.log('🔍 调用DeepSeek API进行快速语言检测...');
    
    // 优化提示词，减少token使用
    const prompt = `识别语言，只返回ISO代码(zh/en/ja/ko/es/fr/de等): "${text.substring(0, 200)}"`;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`DeepSeek语言检测API请求失败: HTTP ${response.status}`);
        }
        
        const data = await response.json();
        const detectedCode = data.choices[0]?.message?.content?.trim().toLowerCase();
        
        console.log(`✅ 语言检测成功: ${detectedCode}`);
        
        // 验证并映射检测到的语言代码
        if (detectedCode && AppState.languages[detectedCode]) {
            return detectedCode;
        }
        return 'unknown'; // 如果API返回无法识别的代码则回退
    } catch (error) {
        clearTimeout(timeoutId);
        
        if (error.name === 'AbortError') {
            console.warn('⚠️ 语言检测超时，使用快速回退');
            return 'unknown';
        }
        
        console.error('❌ DeepSeek语言检测API调用失败:', error);
        throw error;
    }
}
...
async function translateContent(content) {
    ...
    if (useAI) {
        try {
            ...
            // 调用智能翻译API - 设置快速超时
            showNotification('正在使用DeepSeek AI翻译...', 'info');
            
            // 使用Promise.race实现快速超时
            const translationPromise = callDeepSeekAPI(translationPrompt, content);
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error('AI翻译超时，使用快速回退')), 12000)
            );
            ...
            // 快速回退到标准翻译
            showNotification('AI翻译超时，使用快速标准翻译', 'warning');
...
async function callDeepSeekAPI(prompt, originalContent, retryCount = 0) {
    ...
    try {
        console.log(`🚀 调用DeepSeek API (尝试 ${retryCount + 1}/${DEEPSEEK_CONFIG.maxRetries + 1})`);
        ...
        const response = await fetch(`${DEEPSEEK_CONFIG.baseURL}/chat/completions`, {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 2000, // 减少token数量，提高速度
                stream: false
            }),
            signal: controller.signal,
            mode: 'cors',
        ...
        if (!response.ok) {
            ...
            switch (response.status) {
                ...
                case 503:
                    ...
                    break;
                default:
                    userMessage = `API请求失败: ${errorMessage}`;
            }
        ...
        if (retryCount < DEEPSEEK_CONFIG.maxRetries) {
            console.log(`⚠️ API调用失败，${DEEPSEEK_CONFIG.retryDelay}ms后重试...`);
        ...
        console.error('❌ DeepSeek API调用最终失败:', error);
        throw error;
    }
}
...
function buildTranslationPrompt(content, sourceLang, targetLang) {
    ...
    // 简化提示词，提高API响应速度
    let prompt = `翻译${sourceLanguageName}到${targetLanguageName}，保持格式：\n\n`;
    ...
    // 简化JSON格式要求
    prompt += `返回JSON格式：{"paragraphs":[{"type":"paragraph","content":"翻译内容"},{"type":"table","content":{"headers":["表头"],"rows":[["行内容"]]}}]}`;
